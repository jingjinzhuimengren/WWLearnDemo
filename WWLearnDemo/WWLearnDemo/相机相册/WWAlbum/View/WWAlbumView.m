...
typedef void(^LYFAlbumViewSelectAction)(WWAlbumModel *albumModel);

@interface WWAlbumView()
...
/// 相册列表
@property (nonatomic, strong) WWAlbumTableView *tableView;
/// 列表的高度
...
+(void)showAlbumView:(NSMutableArray<WWAlbumModel *> *)assetCollectionList navigationBarMaxY:(CGFloat)navigationBarMaxY complete:(void(^)(WWAlbumModel *albumModel))complete {
    WWAlbumView *albumView = [[WWAlbumView alloc] init];
...
-(void)setAssetCollectionList:(NSMutableArray<WWAlbumModel *> *)assetCollectionList {
    ...
    __weak typeof(self) weakSelf = self;
    self.tableView.selectAction = ^(WWAlbumModel *albumModel) {
...
#pragma mark - Get方法
-(WWAlbumTableView *)tableView {
    if (!_tableView) {
        _tableView = [[WWAlbumTableView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) style:UITableViewStylePlain];
