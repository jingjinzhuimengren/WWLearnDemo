...
#import "WWAlbumCollectionViewCell.h"
#import "WWAlbumModel.h"
#import "WWPhotoManger.h"
#import "WWAlbumView.h"
#import "WWPhotoModel.h"

#define kScreenWidth  [[UIScreen mainScreen] bounds].size.width
#define kScreenHeight [[UIScreen mainScreen] bounds].size.height

@interface WWAlbumViewController () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
...
static NSString *albumCollectionViewCell = @"WWAlbumCollectionViewCell";

@implementation WWAlbumViewController
...
-(void)setupViewController {
    ...
    [WWPhotoManger standardPhotoManger].choiceCountChange = ^(NSInteger choiceCount) {
        ...
        if (choiceCount == 0) {
            ...
            [weakSelf.confirmButton setTitle:[NSString stringWithFormat:@"确定%ld/%ld", [WWPhotoManger standardPhotoManger].choiceCount, [WWPhotoManger standardPhotoManger].maxCount] forState:UIControlStateNormal];
...
-(void)getThumbnailImages {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        for (PHAssetCollection *collection in cameraRolls) {
            WWAlbumModel *model = [[WWAlbumModel alloc] init];
...
#pragma mark - Set方法
-(void)setAlbumModel:(WWAlbumModel *)albumModel {
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    WWAlbumCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:albumCollectionViewCell forIndexPath:indexPath];
    ...
    cell.selectPhotoAction = ^(PHAsset *asset) {
        ...
        if ([weakSelf.albumModel.selectRows containsObject:@(indexPath.row)]) {
            ...
            [WWPhotoManger standardPhotoManger].choiceCount--;
            
            isReloadCollectionView = [WWPhotoManger standardPhotoManger].choiceCount == 9;
        } else {
            if ([WWPhotoManger standardPhotoManger].maxCount == [WWPhotoManger standardPhotoManger].choiceCount) {
                return;
            }
            
            [weakSelf.albumModel.selectRows addObject:@(indexPath.row)];
            [WWPhotoManger standardPhotoManger].choiceCount++;
            isReloadCollectionView = [WWPhotoManger standardPhotoManger].choiceCount == 10;
        }
...
-(void)showAlbum:(UIButton *)button {
    ...
    [WWAlbumView showAlbumView:self.assetCollectionList navigationBarMaxY:CGRectGetMaxY(self.navigationController.navigationBar.frame) complete:^(WWAlbumModel *albumModel) {
        if (albumModel) {
...
-(void)confirmAction:(UIButton *)button {
    if ([WWPhotoManger standardPhotoManger].choiceCount > 0) {
        button.enabled = NO;
        NSMutableArray<WWPhotoModel *> *photoList = [NSMutableArray array];
        
        __weak typeof(self) weakSelf = self;
        for (WWAlbumModel *albumModel in self.assetCollectionList) {
            for (NSNumber *row in albumModel.selectRows) {
                if (row.integerValue < albumModel.assets.count) {
                    WWPhotoModel *photoModel = [[WWPhotoModel alloc] init];
                    
                    __weak typeof(photoModel) weakPhotoModel = photoModel;
                    photoModel.getPictureAction = ^{
                        [photoList addObject:weakPhotoModel];
                        
                        if (photoList.count == [WWPhotoManger standardPhotoManger].choiceCount) {
                            button.enabled = YES;

                            [WWPhotoManger standardPhotoManger].photoModelList = photoList;
...
-(UICollectionView *)albumCollectionView {
    if (!_albumCollectionView) {
        ...
        [_albumCollectionView registerClass:[WWAlbumCollectionViewCell class] forCellWithReuseIdentifier:albumCollectionViewCell];
